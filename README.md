...
| HackerRank | Stacks & Queues | [Balanced Brackets]() | $O(N)$ | $O(N)$ | LIFO parsing stack with expected-char matching and $O(1)$ parity/mismatch early exit |
| HackerRank | Stacks & Queues | [Simple Text Editor]() | $O(Q \cdot K)$ | $O(Q \cdot K)$ | Interactive text buffer supporting $O(1)$ random-access prints and $O(K)$ rollback using an inverted delta command stack |
| LeetCode | Arrays & Matrices | [Gas Station]() | $O(N)$ | $O(1)$ | Single-pass greedy sweep with dual-accumulator state decoupling (`totalTank` global conservation vs. `currentTank` candidate reset) |
...
| LeetCode | Bit Manipulation | [Number of 1 Bits]() | $O(k)$ set bits | $O(1)$ | Brian Kernighan's bit-clearing invariant (`n &= (n - 1)`) iterating strictly per set bit |
| LeetCode | Bit Manipulation | [Single Number III]() | $O(N)$ | $O(1)$ | Cumulative XOR partitioning using two's complement lowest set bit isolation (`diff & -diff`) |
...
| LeetCode | Stacks, Queues, and Strings | [Design Circular Deque]() | $O(1)$ all ops | $O(K)$ | Contiguous fixed-size ring buffer with bidirectional modular index wrapping |
